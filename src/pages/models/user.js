...
function createWebSocket(url, data, companyId, fromAgent, dispatch){
    ...
    ws.onopen = function(){
        //中台商
        if ( data.agent_id ){
            ws.send(`combustagent:${data.agent_id}`);
        } else {
            // 企业端
            ws.send(`combust:${companyId}`);       
        }
    };
    // ws.onclose = function(){
    ...
    effects:{
        *userAuth(action, {call, select, put, all}){ 
            try {
                ...
                if ( !authorized ){
                    ...
                    let { data } = yield call( matchResult ? agentUserAuth : userAuth, matchResult ? { app_type:5, company_id } : { app_type:5 } );
        ...
        *login(action,{ call, put, select }){
            try {
                ...
                var { data }  = yield call(login, { user_name, password, app_type:'5' });
                if ( data && data.code === '0'){   
